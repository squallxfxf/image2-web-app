...
export function PromptEditor({
  ...
  return (
    <div className="space-y-2">
      {editing ? <textarea className="w-full rounded border p-2" value={text} onChange={(e) => setText(e.target.value)} /> : <p className="whitespace-pre-wrap text-sm">{text}</p>}
      <div className="flex flex-wrap gap-2 text-xs">
        <button onClick={() => navigator.clipboard.writeText(text)} className="rounded border px-2 py-1">复制</button>
        <button onClick={() => setEditing((v) => !v)} className="rounded border px-2 py-1">编辑</button>
        <button onClick={save} className="rounded border px-2 py-1">保存</button>
        <button onClick={() => setShowOpt((v) => !v)} className="rounded border px-2 py-1">AI优化</button>
      </div>
